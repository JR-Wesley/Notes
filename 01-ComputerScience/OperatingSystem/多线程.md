...
[//1.通过类的静态对象](https://1.xn--cetr7g7szrofs9v9ueuqa177b/)，单例模式中使用类的全局唯一实例来访问类成员

[//2.类对象作为参数传递给该静态函数中](https://2.xn--fiqza44d2g93jv9csvv9shw7mda4995d76fus1coobf11azl2a/)
...
[task.fun](http://task.fun/)(task.arg);
...
[//1.先将根节点放到队列中](https://1.xn--fiq98e0r9c158cuonu2enwt0x1bx83b/)

[//2.根节点弹出队列](https://2.xn--79qqc734ckxi4ukr00ako4a/)，然后将根节点的左、右儿子入队

[//3.弹出左儿子](https://3.xn--v4qqrk5uvtcn3b/)，放入左儿子的左右儿子

[//4.弹出右儿子](https://4.xn--v4qqr12cq9m54e/)，放入右儿子的左右儿子

[//5.重复3](https://5.xn--3-n90b765o/)、4步
...
m_data = [other.data](http://other.data/);

[other.data](http://other.data/) = nullptr;
...
[参考链接]([C++ 实现多线程打印奇偶数-CSDN博客](https://blog.csdn.net/Deep___Learning/article/details/106013837))

[参考链接]([C/C++语言实现两个线程交替打印奇偶数_c语言多线程循环打印奇偶数-CSDN博客](https://blog.csdn.net/wkb15002993591/article/details/107046097))
...
[//2.LRUCache这个类实现双向链表的添加](https://2.xn--lrucache-0c0mm28ajhdxxao57eqy2dc5rblktqzi82c7s3a0rv/)，删除，更新和遍历

[//3.同时这个类还要实现get和put两个功能](https://3.xn--getput-nu7ilbh425ip1d9vbg93cf96acb7b8n3a49p199abgzra/)

[//4.我这里用的是循环双向链表](https://4.xn--jor2fn65ajmcurhrrxllctwd4s5dctp36exql/)，因此查找链表尾端的元素为O(1)，正常的双向链表是O(n)
...
[//1.之所以用deque不用list是因为移除尾部元素的时候](https://1.xn--dequelist-pc6ntphg00fs7gyrf491ao1uuwyfeqp6ar70sga721ffowc5r1k5jb74a/)，deque方便

[//2.deque里面可以存储自定的node类型](https://2.xn--dequenode-f37n78mvxln9lbmnl2az362a2gvaglwe52efmxa/)，也可以用pair表示，这里我用pair了
